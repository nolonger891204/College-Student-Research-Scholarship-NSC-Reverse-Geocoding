...
def placeProcess(undefined_place):
  if ( type(undefined_place) == str ):
    return {1: [undefined_place]}
  elif ( type(undefined_place) == dict ):
    print(undefined_place["wkt"])
    name_list = getPlaceName(undefined_place["wkt"], undefined_place["datatype"]) # undefined_place, geom_type
    return name_list
  else:
    print("unknown input!")
